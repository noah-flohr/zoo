...
@factory
class TernaryResNetE18Factory(ModelFactory):
    """Implementation of [TernaryResNetE18](https://arxiv.org/abs/1906.08637)"""
    ...
    def build(self) -> tf.keras.models.Model:
        ...
        model = tf.keras.Model(
            inputs=self.image_input,
            outputs=x,
            name=f"ternary_resnet_e_{self.num_layers}",
        )
...
def TernaryResNetE18(
    *,  # Keyword arguments only
    ...
    """Instantiates the TernaryResNetE 18 architecture.
    ...
    ```
    ```summary
    literature.TernaryResNetE18
    ```
    ```plot-altair
    /plots/resnet_e_18.vg.json
    ```

    # ImageNet Metrics for Binary Counterpart
    ...
    # References
        - [Back to Simplicity: How to Train Accurate BNNs from
            Scratch?](https://arxiv.org/abs/1906.08637)
    """
    return TernaryResNetE18Factory(
        input_shape=input_shape,
        input_tensor=input_tensor,
        weights=weights,
        include_top=include_top,
